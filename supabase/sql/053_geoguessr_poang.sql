-- ============================================================
-- Kartan — GeoGuessr-stil poängsättning
-- Höjer maxpoäng från 1000 till 5000, och gör avtagningskurvan
-- mycket mildare (decay-multiplikator 6 -> 20) så att två fel
-- gissningar som ligger olika långt bort faktiskt känns olika
-- rätt/fel, istället för att båda kollapsar mot ~0.
--
-- Exempel (facit = Luleå): en gissning i Gävle (470 km bort)
-- gav tidigare 44 poäng, en gissning i Göteborg (900 km bort)
-- gav 2 poäng — båda kändes som "noll". Med nya kurvan blir det
-- 1953 respektive 826 poäng: närmare lönar sig tydligt, men
-- städernas tighta paket (tolerans 0.5-2 km) straffar fortfarande
-- den som gissar 30+ km fel rejält (249 poäng).
--
-- Kör EFTER 001-052.
-- ============================================================
...
begin
  ...
  if v_runda.typ in ('lan', 'kommun') then
    ...
    v_poang := case when v_korrekt then 5000 else 0 end;
    ...
    -- decay = tolerans × 20 (tidigare × 6), golv 0.3 km för att
    -- undvika extremvärden vid väldigt tighta stadsfrågor.
    v_decay_km := greatest(v_runda.tolerans_km, 0.3) * 20;
    v_poang := greatest(0, least(5000, round(5000 * exp(-v_avstand_km / v_decay_km))::integer));
