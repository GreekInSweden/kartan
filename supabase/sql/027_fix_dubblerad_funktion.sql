-- ============================================================
-- Kartan — fix: dubblerad submit_kartan_guess-funktion
-- Uppstod eftersom "create or replace function" med en NY
-- parameterlista (p_paket_id tillkom) skapar en överlagrad
-- funktion i Postgres istället för att ersätta den gamla —
-- de räknas som olika funktioner så länge signaturen skiljer.
-- Säker att köra oavsett hur långt 026 kom innan den kraschade.
-- ============================================================

-- Ta bort båda möjliga signaturerna, oavsett vilka som finns
drop function if exists submit_kartan_guess(uuid, uuid, text, double precision, double precision);
drop function if exists submit_kartan_guess(uuid, uuid, text, double precision, double precision, uuid);

-- Skapa om, med paket-stödet, som enda kvarvarande version
...
-- Nu finns bara EN version kvar, så namnet räcker för att peka ut den
revoke all on function submit_kartan_guess(uuid, uuid, text, double precision, double precision, uuid) from public;
grant execute on function submit_kartan_guess(uuid, uuid, text, double precision, double precision, uuid) to authenticated, anon;
